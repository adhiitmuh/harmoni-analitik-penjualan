...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .nav-btn{{padding:13px 19px;font-size:.82rem;font-weight:600;cursor:pointer;border:none;background:transparent;color:#94a3b8;border-bottom:3px solid transparent;white-space:nowrap;transition:all .2s;font-family:'Raleway',sans-serif}}
  ...
  .tr{{text-align:right;font-family:'SF Mono',monospace}}
  ...
  .filter-row select,.filter-row input{{padding:6px 11px;border:1px solid rgba(3,69,67,.2);border-radius:7px;font-size:.8rem;background:white;color:var(--black);cursor:pointer;font-family:'Raleway',sans-serif;outline:none}}
  ...
  .pbtn{{padding:4px 13px;border-radius:20px;border:1.5px solid rgba(3,69,67,.25);background:white;font-size:.75rem;font-weight:600;color:var(--g);cursor:pointer;font-family:'Raleway',sans-serif;transition:all .15s}}
...
<div class="nav">
  ...
  <button class="nav-btn" onclick="showTab('stok',this)">📦 Stok & Rekomendasi</button>
</div>
...
<script>
const BULAN = {js_bulan};
const TOTAL_BULANAN         = {js_total_bulanan};
const TOTAL_PROFIT_BULANAN  = {js_total_profit_bulanan};
const TOTAL_HPP_BULANAN     = {js_total_hpp_bulanan};
const YH_OMZET   = {js_yh_omzet};
const YH_PROFIT  = {js_yh_profit};
const YH_MARGIN  = {js_yh_margin};
const CUST_OMZET = {js_cust_omzet};
const KONTR_YH   = {js_kontr_yh};
const KAT_NAMES          = {js_kat_names};
const KAT_MONTHLY        = {js_kat_monthly};
const KAT_MONTHLY_PROFIT = {js_kat_monthly_profit};
const KAT_MONTHLY_HPP    = {js_kat_monthly_hpp};
const KAT_DIVISI         = {js_kat_divisi};
const RINGKASAN   = {js_ringkasan};
const DIVISI_DATA = {js_divisi_data};
const TOP_PRODUK  = {js_top_produk};
const MASALAH_SKU = {js_masalah};
const STOK_DATA   = {js_stok};
...
Chart.defaults.font.family="'Raleway',-apple-system,BlinkMacSystemFont,sans-serif";
...
let cTopOmzet = new Chart(document.getElementById('cTopOmzet'),{{type:'bar',indexAxis:'y',data:{{labels:[],datasets:[{{label:'Omzet',data:[],backgroundColor:'#034543',borderRadius:4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>' '+fmtM(c.raw)}}}}}},scales:{{x:{{ticks:{{callback:v=>fmtM(v)}},grid:{{color:'#f1f5f9'}}}},y:{{grid:{{display:false}},ticks:{{font:{{size:10}}}}}}}}}}}}}});
let cTopMargin = new Chart(document.getElementById('cTopMargin'),{{type:'bar',indexAxis:'y',data:{{labels:[],datasets:[{{label:'Margin%',data:[],backgroundColor:'#1a8b88',borderRadius:4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>' '+c.raw.toFixed(1)+'%'}}}}}},scales:{{x:{{ticks:{{callback:v=>v+'%'}},max:110,grid:{{color:'#f1f5f9'}}}},y:{{grid:{{display:false}},ticks:{{font:{{size:10}}}}}}}}}}}}}});
...
function renderStok(data){{
  ...
  data.forEach((s,i)=>{{
    ...
    tb.innerHTML+=`<tr>
      ...
      <td class="tr" style="font-weight:600">${{s.total_qty.toLocaleString('id-ID')}}</td>
      <td class="tr">${{s.avg_per_bulan}}</td>
      <td class="tr">${{s.recent3_avg}}</td>
      <td>${{trendBadge(s.trend)}}</td>
      <td class="tr" style="color:${{tcolor}};font-weight:600">${{tpct}}</td>
